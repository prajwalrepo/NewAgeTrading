# =============================================================================
# INTRANATGASMINI.PY - Intraday NATGASMINI option strategy for Groww API
#
# This is a dedicated NATGASMINI version based on the pattern used in the
...
# 2) Select ATM strike using a live/near-live underlying NATGASMINI price.
...
CONFIG = {
   ...
   "api_key": "eyJraWQiOiJaTUtjVXciLCJhbGciOiJFUzI1NiJ9..5lRo2nlPb4v5xdAp038NbKL7F5vkFFijwMNXrV4vLghxAAWrMQkAZx3CVx8zqIyySd5AQXzHTdT_-lAWB2lJLg",
    ...
    "expiry_fs": ["23Sep26", "23Oct26"],
    "order_expiry": ["23SEP", "23OCT"],
    ...
    "smtp_use_tls": True,
    "log_dir": "logs",
    "log_file_prefix": "INTRANATGASMINI",
}
...
def _normalize_expiry_entries(expiry_key="expiry_fs", order_key="order_expiry"):
    ...
    for idx, item in enumerate(expiry_list):
        ...
        if expiry_dt is None:
            print(f"[WARN] Invalid expiry format '{candle_expiry}' for NATGASMINI")
            continue
        entries.append({
    ...
    while True:
        try:
            ...
            if underlying_price is None:
                print("[WARN] Underlying NATGASMINI price unavailable yet; retrying with fallback option scan")
                time.sleep(30)
                continue

            print(f"[UNDERLYING] NATGASMINI Price={underlying_price:.2f}")
            if future_symbol:
            ...
            for side in ("CE", "PE"):
                ...
                if (
                    ...
                    if exit_price is not None:
                        print(
                            f"[{idx_time}] [SELL SIGNAL] NATGASMINI {side} "
                ...
                if ok and snapshot.get("signal") == "BUY":
                    ...
                    print(f"[{idx_time}] [SIGNAL] NATGASMINI {side} {signal_symbol} Close={close_price:.2f} ST={snapshot.get('st_value') if snapshot.get('st_value') is not None else 'NA'}")
...
def main():
    ...
    tee_stdout = DailyTeeStdout(
        ...
        file_prefix=str(CONFIG.get("log_file_prefix") or "INTRANATGASMINI"),
    )
    ...
    print("[INIT] INTRANATGASMINI started")
    if not build_contract_universe():
        raise SystemExit(1)
