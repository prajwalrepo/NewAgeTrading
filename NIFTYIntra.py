# =============================================================================
# NIFTYFNOInatra.py - NIFTY intraday FNO Supertrend strategy on Groww API
#
# Flow:
# 1) Read NIFTY index signal first.
# 2) Use current index price to select the nearest ATM strike.
# 3) Build CE/PE candle symbols in the form:
#       NIFTY-{FetchingExpiry}-24500-CE
#    and order symbols in the form:
#       NIFTY{orderexpiry}24500CE
...
def place_buy_order(candle_symbol, order_symbol, price, entry_candle_open, qty=None, entry_candle_low=None):
    try:
        ...
        use_margin = min(float(CONFIG.get("allocation_per_trade", 0)), fno_margin or float(CONFIG.get("allocation_per_trade", 0)))
...
def option_has_flip_down_exit_signal(candle_symbol, idx_time):
    ...
    signal = "SELL" if fresh_flip_down else "NO_SIGNAL"
    return fresh_flip_down, close_price, st_value, signal
...
def option_has_flip_buy_signal(candle_symbol, idx_time):
    ...
    if candle_time != idx_time:
        st_value = float(st_arr[closed_idx]) if not np.isnan(st_arr[closed_idx]) else None
        return False, float(df["close"].iloc[closed_idx]), float(df["open"].iloc[closed_idx]), float(df["low"].iloc[closed_idx]), st_value, None, "NO_SIGNAL"
        ...
        try:
            ...
            for symbol, pos in list(positions.items()):
                ...
                if should_exit and exit_signal == "SELL":
                    if CONFIG.get("DEBUG"):
                        st_txt = f"{st_value:.2f}" if st_value is not None else "NA"
                        print(f"[DEBUG] Flip exit {symbol} | Signal={exit_signal} | ST={st_txt}")
                    place_sell_order(
                        symbol,
                        pos.get("order_symbol"),
                        float(exit_price),
            ...
            if has_open_on_side("CE") or has_live_open_on_side("CE", live_positions) or has_open_on_side("PE") or has_live_open_on_side("PE", live_positions):
                ...
                if not atm:
                    ...
                    ce_ok = bool(ce_snapshot and ce_snapshot.get("ok") and ce_snapshot.get("last_price") is not None and ce_snapshot.get("entry_open") is not None)
                    pe_ok = bool(pe_snapshot and pe_snapshot.get("ok") and pe_snapshot.get("last_price") is not None and pe_snapshot.get("entry_open") is not None)
