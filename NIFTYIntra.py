        ...
        try:
            ...
            if session_state != last_session_state_logged:
                if session_state == "closed_day":
                    print(
                        f"[SESSION] Market closed today. Allowed days={CONFIG.get('trading_weekdays')} "
                        f"Current IST={now_ist.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                elif session_state == "pre_open":
                    print(
                        f"[SESSION] Outside market hours. Trading starts at {CONFIG.get('entry_window_start')} IST. "
                        f"Current IST={now_ist.strftime('%Y-%m-%d %H:%M:%S')}"
                    )
                elif session_state == "manage_only":
                    print(
                        f"[SESSION] Entry window closed at {CONFIG.get('entry_window_end')} IST. "
                        f"Managing open positions until {CONFIG.get('force_exit_time')} IST."
                    )
                elif session_state == "squareoff":
                    print(
                        f"[SESSION] Square-off window active at {CONFIG.get('force_exit_time')} IST. "
                        "Closing open positions only."
                    )
                elif session_state == "entry":
                    print(
                        f"[SESSION] Trading window open: {CONFIG.get('entry_window_start')} - "
                        f"{CONFIG.get('entry_window_end')} IST."
                    )
